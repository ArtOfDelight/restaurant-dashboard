...
app.post('/api/product-chat', async (req, res) => {
  try {
    ...
    // Detect stock-out focused queries (handle without AI)
    const isStockOutQuery = /(?:which|what|show|list|tell).{0,30}(?:items?|products?).{0,30}(?:went|gone|was|were|are).{0,30}(?:out of stock|stock out|unavailable)|(?:out of stock|stock out).{0,30}(?:items?|products?|list)|stock.{0,20}(?:events?|issues?)/i.test(message);

    if (isStockOutQuery) {
      console.log('🎯 Stock-out query detected - handling with direct data (no AI)');

      // Parse days back from message or use default
      const daysBack = dateQuery?.days || dateFilter || 7;
      const outlet = filters.branch || null;

      // Get stock events directly
      const stockData = await getAllStockEvents(daysBack, outlet);

      if (stockData.stockEvents.length === 0) {
        return res.json({
          success: true,
          response: `No out-of-stock events were detected in the last ${daysBack} days${outlet ? ` at ${outlet}` : ''}.`,
          data: {
            summary: {
              totalEvents: 0,
              uniqueProducts: 0,
          ...
          dateRangeInfo: `Last ${daysBack} days`,
          timestamp: new Date().toISOString()
        });
      }
      ...
      let response = `Found ${stockData.stockEvents.length} out-of-stock event(s) across ${productList.length} product(s) in the last ${daysBack} days${outlet ? ` at ${outlet}` : ''}.\n\n`;
      response += `=== OUT-OF-STOCK ITEMS ===\n\n`;

      productList.forEach((product, idx) => {
        response += `${idx + 1}. ${product.itemName}\n`;
        response += `   Total Events: ${product.totalEvents}\n`;

        // Show first 3 events for this product
        const eventsToShow = product.events.slice(0, 3);
        eventsToShow.forEach((event, eventIdx) => {
          response += `   ${eventIdx + 1}) SKU ${event.sku} - ${event.dateTime} at ${event.outlet}\n`;
        });

        if (product.events.length > 3) {
          response += `   ... and ${product.events.length - 3} more event(s)\n`;
        }
        response += `\n`;
      });
