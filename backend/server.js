...
const bot = new TelegramBot(TELEGRAM_BOT_TOKEN, { polling: true }); // Use polling for now
const BROADCAST_SPREADSHEET_ID = CHECKLIST_SPREADSHEET_ID; // Use same spreadsheet
const BROADCAST_TAB = 'Broadcasts';

// Initialize Broadcasts tab in Google Sheet
...
// Call initialization after Google Sheets setup
initializeGoogleSheets().then(() => initializeBroadcastTab());
...
// Helper function to generate unique broadcast ID
function generateBroadcastId() {
  return `BCAST-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}

// Send Telegram broadcast and store in Google Sheet
app.post('/api/send-broadcast', async (req, res) => {
  try {
    ...
    if (!message || !Array.isArray(recipients) || recipients.length === 0) {
      return res.status(400).json({
        success: false,
        error: 'Missing message or recipients array',
      });
    }

    console.log(`📢 Sending broadcast to ${recipients.length} recipients: ${message}`);

    const broadcastId = generateBroadcastId();
    const timestamp = new Date().toISOString();
    const values = [];

    // Send message to each recipient and prepare Google Sheet data
    for (const recipient of recipients) {
      const { user, chatId } = recipient;
      if (!user || !chatId) {
      ...
      try {
        await bot.sendMessage(chatId, message, {
          reply_markup: {
            inline_keyboard: [[
              {
                text: 'Understood',
                callback_data: JSON.stringify({ broadcastId, chatId }),
              },
            ]],
          },
        });
        ...
        console.log(`✅ Sent broadcast to ${user} (${chatId})`);
      } catch (error) {
        console.error(`❌ Error sending to ${user} (${chatId}):`, error.message);
        values.push([
          broadcastId,
          message,
          timestamp,
    ...
    console.log(`✅ Stored ${values.length} broadcast records in ${BROADCAST_TAB}`);
...
// Handle Telegram callback queries for acknowledgments
bot.on('callback_query', async (callbackQuery) => {
  ...
  try {
    ...
    return bot.answerCallbackQuery(callbackQuery.id, { text: 'Error processing acknowledgment' });
  }

  const { broadcastId, chatId: recipientChatId } = data;
  if (chatId !== recipientChatId) {
    console.warn(`Unauthorized callback from chatId ${chatId} for broadcast ${broadcastId}`);
    return bot.answerCallbackQuery(callbackQuery.id, { text: 'Unauthorized' });
  }

  try {
    // Find the broadcast record in Google Sheet
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: BROADCAST_SPREADSHEET_ID,
      range: `${BROADCAST_TAB}!A:G`,
    });
    ...
    for (let i = 1; i < rows.length; i++) {
      if (rows[i][0] === broadcastId && rows[i][4] === chatId.toString()) {
        targetRow = i + 1; // 1-based indexing for Sheets
        break;
      }
    }

    if (targetRow === -1) {
      console.warn(`Broadcast ${broadcastId} for chatId ${chatId} not found`);
      return bot.answerCallbackQuery(callbackQuery.id, { text: 'Broadcast not found' });
    }

    // Update status to Acknowledged and set timestamp
    ...
    await sheets.spreadsheets.values.batchUpdate({
      ...
      resource: {
        data: [
          {
            range: `${BROADCAST_TAB}!F${targetRow}`,
            values: [['Acknowledged']],
          },
          {
            range: `${BROADCAST_TAB}!G${targetRow}`,
    ...
    console.log(`✅ Acknowledged broadcast ${broadcastId} by chatId ${chatId}`);
    bot.answerCallbackQuery(callbackQuery.id, { text: 'Thank you for acknowledging!' });
  } catch (error) {
    console.error('❌ Error handling acknowledgment:', error.message);
    bot.answerCallbackQuery(callbackQuery.id, { text: 'Error processing acknowledgment' });
  }
});
...
app.get('/api/broadcast-history', async (req, res) => {
  try {
    ...
    const broadcasts = {};
    for (let i = 1; i < rows.length; i++) {
      const [id, message, timestamp, user, chatId, status, acknowledgedAt] = rows[i];
      if (!broadcasts[id]) {
        broadcasts[id] = {
          id,
          message,
    ...
    const broadcastList = Object.values(broadcasts);
    console.log(`✅ Retrieved ${broadcastList.length} unique broadcasts`);
